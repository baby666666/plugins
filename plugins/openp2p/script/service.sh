...
start() {
    ...
    chmod +x /usr/sbin/openp2p
    [ -L "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/openp2p" ] || ln -fs /usr/sbin/openp2p "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/openp2p"
    $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/openp2p -d -node "$node" -token "$token" >/tmp/iktmp/plugins/$PLUGIN_NAME/openp2p.log 2>&1 &
    return 0
}
...
__show_data() {
    ...
    [ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/node" ] && node=$(cat $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/node)
    [ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/token" ] && token=$(cat $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/token)
